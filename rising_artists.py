...
# Funktionen zur Abfrage von Song-Daten aus der Songs-Datenbank
...
# Hole alle Song-Seiten aus der Songs-Datenbank
def get_all_song_page_ids():
    ...
    for page in data.get("results", []):
        page_id = page["id"]
        # Popularity aus der Songs-Datenbank (Property "Popularity")
        popularity = 0
        ...
        song_pages.append({"page_id": page_id, "popularity": popularity, "artist": artist})
    return song_pages

# --- Funktion zum Erstellen eines neuen Week-Eintrags (Messung) ---
def create_week_entry(song_id, popularity_score, code):
...
def update_popularity():
    ...
    # Erzeuge einen Run-Code basierend auf dem aktuellen Timestamp
    run_time = datetime.datetime.now().strftime("%Y%m%d-%H%M")
    # Hole alle Song-Seiten
    song_pages = get_all_song_page_ids()
    print(f"Gefundene Songs: {len(song_pages)}")
    # Pro Artist soll ein eindeutiger Code generiert werden
    codes = {}
    for song in song_pages:
        ...
        create_week_entry(song_id=song["page_id"], popularity_score=song["popularity"], code=code)
        time.sleep(0.3)
...
def get_weeks_entries():
    ...
    Holt alle Einträge aus der Weeks-Datenbank und liefert eine Liste von Dictionaries mit:
      - track_code (aus Title-Property "Track Code")
...
def build_code_graph(code):
    """
    Baut einen Graphen, der alle Einträge (Messungen) mit einem bestimmten Code (im Title-Property "Track Code") anzeigt.
    """
...
# --- Platzhalterfunktion für Get New Music (optional) ---
def get_new_music():
...
Popularity: {row['last_popularity']:.1f} | Growth: {row['growth']:.1f}%""")
            ...
            with st.expander(f"{row['track_name']} - {row['artist']} anzeigen"):
                # Der Nutzer gibt hier den Code ein, nach dem gruppiert werden soll
...
# --- Hauptprogramm: Update und Anzeige ---
st.title("Song Tracking Übersicht")
...
# Anzeige der Top 10 (kumulatives Wachstum über alle Messungen)
st.header("Top 10 Songs – Wachstum über alle Messungen")
tracking_entries = get_tracking_entries()
metadata = get_metadata_from_tracking_db()
df = pd.DataFrame(tracking_entries)
...
# 2. Filterergebnisse: Im Expander kann der Nutzer einen Code eingeben, um den Graphen der zugehörigen Messungen anzuzeigen.
st.header("Songs filtern")
if submitted:
    last_data = []
    for song_id, group in df.groupby("song_id"):
