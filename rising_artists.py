...
st.title("Debug: Messwerte für 'Erfolg ist kein Glück'")
...
def get_weeks_data():
    ...
    Extrahiert:
      ...
      - die Notion Track ID (die den Song eindeutig identifiziert).
    """
...
# Daten abrufen und DataFrame erstellen
data = get_weeks_data()
df = pd.DataFrame(data)
st.write("**Rohdaten aus Notion (unbearbeitet):**", df)

# Datum parsen
df["date"] = pd.to_datetime(df["date"], errors="coerce")
df = df.dropna(subset=["date"])
st.write("**Daten nach Datumskonvertierung:**", df)

# Filtere alle Einträge für die Notion Track ID "7ocanzdHHuxnYCHgS40CPF"
df_filtered = df[df["notion_track_id"] == "7ocanzdHHuxnYCHgS40CPF"]
st.write("**Alle Einträge für Notion Track ID '7ocanzdHHuxnYCHgS40CPF':**", df_filtered)
