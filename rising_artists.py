...
def get_tracking_entries():
    """Holt Einträge aus der Tracking-Datenbank und gibt sie als Liste zurück."""
    ...
    for page in data.get("results", []):
        ...
        # "Date" ist dein Datums-Property in Notion
...
def get_spotify_data(spotify_track_id):
    """Liefert (Cover-URL, Spotify-Link) gecacht."""
...
# --- Sidebar: Buttons + Filterformular ---
with st.sidebar:
...
# Versuche, Datum + Uhrzeit automatisch zu parsen
...
for song_id, group in df_2days.groupby("song_id"):
    ...
    if first_pop and first_pop != 0:
        growth = ((last_pop - first_pop) / first_pop) * 100
...
for row_df in rows:
    ...
    for idx, (_, row) in enumerate(row_df.iterrows()):
        cover_url, spotify_link = get_spotify_data(row["spotify_track_id"]) if row["spotify_track_id"] else ("", "")
        with cols[idx]:
            if cover_url:
